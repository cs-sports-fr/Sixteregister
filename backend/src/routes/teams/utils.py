        ...
        if general_config.mailClient == mailClient.SES:
            ...
            try:
                email_client.send_email(
                    to_address=email,
                    subject="[TOSS 2024] Charte participant",
...
async def send_host_rez_email(
    ...
    general_config = await prisma.generalconfig.find_first()
    html_template = get_html_template_file("mail_hebergeur.html")
    txt_template = get_html_template_file("mail_hebergeur.txt")
    content = {
        "FIRSTNAME": firstname,
    }
    html_content = fill_template(html_template, content)
    ...
    if general_config.canSendEmails == True:
        if general_config.mailClient == mailClient.SES:
            ...
            try:
                email_client.send_email(
                    to_address=email,
                    subject="[TOSS 2024] Accord d'hebergement",
                    body_html=html_content,
                    body_text=txt_content,
                )
            except Exception:
